...
function buildEnrichmentPrompt(context: ContextBundle, explanation: Explanation): ModelRequest {
  ...
  return {
    system: [
      ...
      "Write like a patient tutor: concrete, short, no UI chatter, no meta commentary about tools/modes.",
      "Return strict JSON only."
    ].join(" "),
    user: [
      "TASK:",
      "Turn the deterministic facts into a learn-codebase-style tutoring card.",
      "",
      "MAP FIELDS EXACTLY LIKE THIS:",
      "- whyItExists: one Purpose paragraph (what this symbol is used for in the app/module)",
      "- whatItDoes: a markdown table of fields/params: | Field | Meaning |",
      "- howItWorks: docstring notes, validators, and 1-2 valid construction shapes/examples grounded in evidence",
      "- codebaseUsage: brief usage bullets if references exist, ending with: Ask about that, or keep moving.",
      "- summary: short title line naming the symbol",
      "- caveats: [] unless truly necessary",
      "",
      "Tone: plain tutoring prose. No pills, toggles, enrichment status, or 'Codegraph identified'.",
      "",
      "TARGET:",
...
export function buildAgentHandoffPrompt(result: SelectionContextResultLike): string {
  ...
  return [
    "You are a calm codebase tutor (learn-codebase style) for a Cursor/Claude subscription user.",
    "No API keys are needed. Explain from the deterministic facts only.",
    "",
    "Write like this example structure:",
    ...
    "- No UI chatter (no toggles, modes, enrichment status, pills).",
    "- Cite file:line from Sources only.",
